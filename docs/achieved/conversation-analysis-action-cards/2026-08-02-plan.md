...
**Spec:** [`2026-08-01-design.md`](./2026-08-01-design.md)（r3）  
**字典草案:** `scripts/data/v1_dictionary_draft.json`  
**构建:** 后端 `./mvn.sh -q test`（仓库根）；前端 `cd frontend && npm run build`  
**归档位置:** `docs/achieved/conversation-analysis-action-cards/`

**本计划范围 = MVP / P0。** 不做：`user_focus_point`、回检条、LLM「练 3 句」（回检备忘见 `docs/todo/focus-recheck/`）。
